...
export class ComparatorComponent implements OnInit {
  ...
  this.selectedNames.forEach((selected, index) => {
    ...
    if (name) {
      ...
      if (scpiDetails && scpiDetails.minimumSubscription > this.investValue && this.investValue > 0) {
        ...
        this.popupMessage = `La SCPI "${name}" a été retirée car le montant minimum de souscription est ${scpiDetails.minimumSubscription}€.`;
        this.showPopup = true;
      }
    }
  });
    ...
    this.detailsService.getDetailsByName(name).subscribe({
      next: (data: Details) => {
        if (data.minimumSubscription > this.investValue) {
          ...
          this.updateFilteredNames();
          this.saveToLocalStorage();
          this.popupMessage = `La SCPI "${name}" a été retirée car le montant minimum de souscription est ${data.minimumSubscription}€.`;
          this.showPopup = true;
          return;
        }
  ...
  formatNumber(value: number): string {
    return new Intl.NumberFormat('fr-FR').format(value); // ex : 1 000
  }
